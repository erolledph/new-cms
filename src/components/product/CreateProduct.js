...
export class CreateProduct {
  ...
  render() {
    ...
    this.element.className = 'create-product-container';
          ...
          <form id="create-product-form">
            ...
            <div class="form-actions">
              ...
              <button type="submit" class="create-button" id="create-button">
                <span class="button-text">Create Product</span>
  ...
  attachEventListeners() {
    ...
    const form = this.element.querySelector('#create-product-form');
  ...
  showProductForm() {
    ...
    const defaultCurrency = this.productSite.defaultCurrency || this.userSettings?.currency || 'USD';
    const currencySelect = this.element.querySelector('#currency-select');
  ...
  async handleCreateProduct() {
    ...
    try {
      ...
      // Navigate back to product site management after delay
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('navigate-to-section', { 
          detail: { section: `manage-product-site-${this.productSiteId}` } 
        }));
  ...
  async handleSaveDraft() {
    ...
    if (!formData.name.trim()) {
      this.showFieldError('product-name-error', 'Product name is required even for drafts.');
      return;
    }
  ...
  async updateProductSiteCount(increment_value) {
    try {
      // Update the product count in the user's productSites array
      const userDocRef = doc(db, 'users', this.currentUser.uid);
      const userDoc = await getDoc(userDocRef);
      
      if (userDoc.exists()) {
        const userData = userDoc.data();
        const updatedProductSites = userData.productSites.map(site => {
          if (site.id === this.productSiteId) {
            return {
              ...site,
              productCount: (site.productCount || 0) + increment_value
            };
          }
          return site;
        });
