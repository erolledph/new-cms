...
export class EditProduct {
  ...
  populateForm() {
    ...
    if (this.productData.imageUrl) {
      this.selectedImageUrls.push(this.productData.imageUrl);
    }
    if (this.productData.imageUrls && this.productData.imageUrls.length > 0) {
      this.selectedImageUrls.push(...this.productData.imageUrls);
    }
  ...
  handleFilesSelected(files) {
    ...
    const newUrls = files.map(file => file.downloadURL);
    this.selectedImageUrls = [...this.selectedImageUrls, ...newUrls].slice(0, 5);
    this.renderSelectedImages();
  }
